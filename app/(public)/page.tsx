...
async function loadData() {
  ...
  for (const p of winnerPurchases ?? []) {
    purchaseById.set(p.id as string, {
      buyer_first_name: (p as any).buyer_first_name,
      buyer_last_name: (p as any).buyer_last_name,
      buyer_club: (p as any).buyer_club
    });
  }
    ...
    <main>
      ...
      <section className="bg-tombola-hero text-white">
        <div className="mx-auto max-w-6xl px-6 py-16 sm:py-24">
          <p className="mb-3 inline-block rounded-full bg-white/10 px-4 py-1 text-sm font-medium tracking-wide backdrop-blur">
          ...
          <p className="mt-4 max-w-2xl text-lg text-emerald-100 sm:text-xl">
            Chaque ticket porte le nom d&apos;un joueur. Champions de France, têtes
            de série régionales, et participants du tournoi : choisissez vos
            favoris et tentez votre chance pour des lots exceptionnels.
          </p>
          <div className="mt-8 flex flex-wrap gap-3 text-sm">
            <span className="rounded-full bg-white/15 px-4 py-2">🎫 1 ticket = 2 €</span>
            <span className="rounded-full bg-white/15 px-4 py-2">🎁 Pack de 3 = 5 €</span>
            <span className="rounded-full bg-abil-gold px-4 py-2 font-semibold text-abil-ink">
              ⭐ Pack de 5 = 7 €
            </span>
          </div>
        </div>
      ...
      <section className="mx-auto max-w-6xl px-6 py-12">
        <header className="mb-8">
          <h2 className="text-3xl font-black sm:text-4xl">Les lots à gagner</h2>
      ...
      {bundles.length > 0 && (
        <section className="mx-auto max-w-6xl px-6 pb-4">
          <h2 className="mb-4 text-2xl font-black">Tickets spéciaux ×5</h2>
      ...
      <section className="mx-auto max-w-6xl px-6 py-12">
        <header className="mb-6 flex flex-wrap items-end justify-between gap-3">
          <div>
            <h2 className="text-3xl font-black sm:text-4xl">Choisissez vos tickets</h2>
            <p className="mt-2 text-slate-600">
              Chaque ticket porte le nom d&apos;un joueur unique. Cliquez pour
